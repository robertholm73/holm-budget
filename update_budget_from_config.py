...
def update_budget_categories():
    ...
    for user, categories in budget_categories.items():
        for category, amount in categories.items():
            ...
            cur.execute("SELECT id, budgeted_amount FROM budget_categories WHERE name = %s", (category_name,))
            result = cur.fetchone()
            ...
            if result:
                ...
                if float(current_amount) != new_amount:
                    ...
                    print(f"Updated {category_name}: {current_amount} -> {new_amount}")
                    categories_updated += 1
                else:
                    print(f"No change needed for {category_name}: {current_amount}")
            else:
                ...
                print(f"Created missing category: {category_name} with amount {amount}")
                categories_updated += 1
