...
def check_database_state():
    ...
    cur.execute(
        "SELECT id, name, budgeted_amount, current_balance FROM budget_categories ORDER BY name"
    )
    categories = cur.fetchall()
    print(f"\nAll budget categories ({len(categories)} total):")
    for cat in categories:
        print(
            f"  ID {cat[0]}: {cat[1]} (budgeted: {cat[2]}, current: {cat[3]})"
        )
...
def fix_duplicates():
    ...
    for name, category_group in categories_by_name.items():
        ...
        print(
            f"  Keeping ID {keep_id}, merging {len(category_group)} duplicates"
        )
        print(
            f"  Total budgeted: {total_budgeted}, Total current: {total_current}"
        )
        ...
        for dup_id in duplicate_ids:
            ...
            cur.execute(
                "DELETE FROM budget_categories WHERE id = %s", (dup_id,)
            )
