...
def run_migration():
    ...
    try:
        ...
        if period_count > 0:
            print(f"Migration already ran - found {period_count} budget periods")
            response = input("Do you want to recreate periods? (y/N): ").lower()
            if response != 'y':
        ...
        for period in periods:
            ...
            print(f"  Added: {period['period_name']} ({period['start_date']} to {period['end_date']}) [{status}]")
        ...
        print("Step 5: Migrating existing budget categories to current period...")
        ...
        if current_period_result:
            ...
            print(f"  Updated {updated_count} existing categories to current period")
        else:
        ...
        try:
            cursor.execute("ALTER TABLE budget_categories DROP CONSTRAINT budget_categories_name_key")
        ...
        if transfer_purchases:
            print(f"  Found {len(transfer_purchases)} transfer-related purchases to migrate")
            for transfer in transfer_purchases:
                purchase_id, user_name, amount, account_id, description, date = transfer
                
                # Try to extract destination account from description
                originator = 'Robert' if user_name in ['Robert', 'Transfer'] else 'Peanut'
                ...
                print(f"    Removed incorrect transfer purchase: {description}")
        ...
        cursor.execute("SELECT COUNT(*) FROM budget_categories WHERE period_id IS NOT NULL")
        category_count = cursor.fetchone()[0]
        print(f"  - Migrated {category_count} budget categories")
        
        cursor.execute("SELECT period_name FROM budget_periods WHERE is_active = TRUE")
...
def verify_migration():
    ...
    try:
        ...
        # Check active period
        cursor.execute("SELECT period_name, start_date, end_date FROM budget_periods WHERE is_active = TRUE")
        active_period = cursor.fetchone()
        if active_period:
            print(f"Active period: {active_period[0]} ({active_period[1]} to {active_period[2]})")
...
if __name__ == "__main__":
    ...
    response = input("This will modify your database schema. Continue? (y/N): ").lower()
    if response != 'y':
        print("Migration cancelled")
        sys.exit(0)
