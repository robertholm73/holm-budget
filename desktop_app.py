...
class BudgetDesktopApp(QMainWindow):
    def __init__(self):
        ...
        if not self.database_url:
            QMessageBox.critical(
                self, "Error", "DATABASE_URL environment variable not set"
            )
            return
    ...
    def setup_ui(self):
        ...
        for user in ["Robert", "Peanut", "Both"]:
            ...
            tab_button.clicked.connect(
                lambda checked, u=user: self.on_user_filter_clicked(u)
            )
    ...
    def setup_budget_tab(self):
        ...
        self.budget_table.setHorizontalHeaderLabels(
            ["ID", "Category", "Budgeted (R)", "Spent (R)", "Remaining (R)"]
        )
        self.budget_table.setSelectionBehavior(QTableWidget.SelectRows)
        self.budget_table.selectionModel().selectionChanged.connect(
            self.on_budget_selection_changed
        )
    ...
    def load_periods(self):
        ...
        try:
            ...
            for period in periods:
                ...
                tab_button.clicked.connect(
                    lambda checked, p_id=period[0]: self.on_period_tab_clicked(
                        p_id
                    )
                )
            ...
            QMessageBox.warning(
                self,
                "Period Load Error",
                f"Failed to load budget periods: {str(e)}",
            )
        ...
        if has_selection:
            ...
            if id_item and id_item.text().strip():
                try:
                    ...
                    category_name = self.budget_table.item(
                        selected_row, 1
                    ).text()
                    self.budget_entry.setPlaceholderText(
                        f"New budget for {category_name}"
                    )
                    return
                except ValueError:
                    pass
        ...
        try:
            ...
            try:
                ...
                for row, account in enumerate(accounts):
                    self.accounts_table.setItem(
                        row, 0, QTableWidgetItem(str(account[0]))
                    )
                    self.accounts_table.setItem(
                        row, 1, QTableWidgetItem(str(account[1]))
                    )
            ...
            try:
                ...
                for row, cat in enumerate(categories):
                    ...
                    spent = abs(
                        float(cat[3])
                    )  # current_balance is negative when money is spent
                    remaining = budgeted - spent
                    self.budget_table.setItem(
                        row, 0, QTableWidgetItem(str(cat[0]))
                    )
                    self.budget_table.setItem(
                        row, 1, QTableWidgetItem(str(cat[1]))
                    )
                    self.budget_table.setItem(
                        row, 2, QTableWidgetItem(f"R{budgeted:.2f}")
                    )
                    self.budget_table.setItem(
                        row, 3, QTableWidgetItem(f"R{spent:.2f}")
                    )
                ...
                total_budgeted_item = QTableWidgetItem(
                    f"R{total_budgeted:.2f}"
                )
                ...
                total_remaining_item = QTableWidgetItem(
                ...
                QMessageBox.warning(
                    self,
                    "Data Load Warning",
                    f"Failed to load budget categories: {str(e)}",
                )
            ...
            try:
                ...
                for row, purchase in enumerate(purchases):
                    formatted_date = (
                        purchase[6].strftime("%Y-%m-%d %H:%M")
                        if purchase[6]
                        else ""
                    )
                    self.purchases_table.setItem(
                        row, 0, QTableWidgetItem(str(purchase[0]))
                    )
                    self.purchases_table.setItem(
                        row, 1, QTableWidgetItem(str(purchase[1]))
                    )
                    ...
                    self.purchases_table.setItem(
                        row, 3, QTableWidgetItem(purchase[3] or "N/A")
                    )
                    self.purchases_table.setItem(
                        row, 4, QTableWidgetItem(purchase[4] or "N/A")
                    )
                    self.purchases_table.setItem(
                        row, 5, QTableWidgetItem(purchase[5] or "")
                    )
                    self.purchases_table.setItem(
                        row, 6, QTableWidgetItem(formatted_date)
                    )
                ...
                self.purchases_table.setItem(
                    total_row, 0, QTableWidgetItem("")
                )
                self.purchases_table.setItem(
                    total_row, 1, QTableWidgetItem("")
                )
                ...
                self.purchases_table.setItem(
                    total_row, 3, QTableWidgetItem("")
                )
                self.purchases_table.setItem(
                    total_row, 4, QTableWidgetItem("")
                )
                self.purchases_table.setItem(
                    total_row, 5, QTableWidgetItem("")
                )
                total_label = QTableWidgetItem("Total")
                total_label.setFont(bold_font)
    ...
    def cleanup_bad_data(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.critical(
                    self, "Cleanup Error", f"Failed to cleanup: {str(e)}"
                )
    ...
    def update_account_balance(self):
        ...
        if selected_row < 0:
            QMessageBox.warning(
                self, "Selection Error", "Please select an account"
            )
            return

        if not self.balance_entry.text().strip():
            QMessageBox.warning(
                self, "Input Error", "Please enter a balance amount"
            )
            return
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                self.load_data()
                QMessageBox.information(
                    self, "Success", "Account balance updated!"
                )

        except ValueError:
            QMessageBox.critical(
                self, "Input Error", "Please enter a valid number"
            )
        except Exception as e:
            QMessageBox.critical(
                self, "Database Error", f"Failed to update balance: {str(e)}"
            )
    ...
    def update_budget_amount(self):
        ...
        if selected_row < 0:
            QMessageBox.warning(
                self,
                "Selection Error",
                "Please select a budget category from the table first",
            )
            return
        ...
        if not id_item or not id_item.text().strip():
            QMessageBox.warning(
                self,
                "Selection Error",
                "Please select a valid budget category (not the Total row)",
            )
            return
        ...
        if not self.budget_entry.text().strip():
            QMessageBox.warning(
                self, "Input Error", "Please enter a budget amount"
            )
            return
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                cur = conn.cursor()
                cur.execute(
                    "UPDATE budget_categories SET budgeted_amount = %s WHERE id = %s",
                ...
                self.load_data()
                QMessageBox.information(
                    self, "Success", "Budget amount updated!"
                )
            ...
            QMessageBox.critical(
                self, "Database Error", f"Failed to update budget: {str(e)}"
            )
    ...
    def add_account(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "INSERT INTO accounts (name, account_type, balance) VALUES (%s, %s, %s)",
                ...
                self.load_data()
                QMessageBox.information(
                    self, "Success", "Account added successfully!"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Database Error", f"Failed to add account: {str(e)}"
                )
    ...
    def edit_account(self):
        ...
        current_balance = float(
            self.accounts_table.item(selected_row, 2).text().replace("R", "")
        )
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "UPDATE accounts SET name = %s, account_type = %s, balance = %s WHERE id = %s",
    ...
    def delete_account(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                cur.execute(
                    "DELETE FROM purchases WHERE account_id = %s",
                    (account_id,),
                )
                # Delete account
                cur.execute(
                ...
                self.load_data()
                QMessageBox.information(
                    self, "Success", "Account deleted successfully!"
                )
            except Exception as e:
                QMessageBox.critical(
                    self,
        ...
        try:
            ...
            try:
                ...
                QMessageBox.critical(
                    self,
                    "Database Timeout",
                    f"Failed to load accounts for transfer: {str(e)}",
                )
                return
            ...
            if dialog.exec() == QDialog.Accepted:
                ...
                description = (
                    data["description"]
                    or f"Transfer from {data['from_account_name']} to {data['to_account_name']}"
                )
                originator = data.get(
                    "originator", "Robert"
                )  # Default to Robert if not specified
            ...
            QMessageBox.critical(
                self, "Database Error", f"Failed to process transfer: {str(e)}"
            )
        ...
        try:
            ...
            try:
                ...
                QMessageBox.critical(
                    self,
                    "Database Timeout",
                    f"Failed to load accounts for income: {str(e)}",
                )
                return
            ...
            QMessageBox.critical(
                self, "Database Error", f"Failed to process income: {str(e)}"
            )
    ...
    def add_budget_category(self):
        if not self.current_period_id:
            QMessageBox.warning(
                self,
                "No Period Selected",
                "Please select a budget period first.",
            )
            return
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                self.load_data()
                QMessageBox.information(
                    self, "Success", "Budget category added successfully!"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Database Error", f"Failed to add category: {str(e)}"
                )
    ...
    def edit_budget_category(self):
        ...
        if selected_row < 0:
            QMessageBox.warning(
                self, "Selection Error", "Please select a budget category"
            )
            return
        ...
        current_budget = float(
            self.budget_table.item(selected_row, 2).text().replace("R", "")
        )
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "UPDATE budget_categories SET name = %s, budgeted_amount = %s WHERE id = %s",
                ...
                self.load_data()
                QMessageBox.information(
                    self, "Success", "Budget category updated successfully!"
                )
            except Exception as e:
                QMessageBox.critical(
                    self,
    ...
    def delete_budget_category(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                cur.execute(
                    "UPDATE purchases SET budget_category_id = NULL WHERE budget_category_id = %s",
                    (category_id,),
                )
                # Delete category
                cur.execute(
                    "DELETE FROM budget_categories WHERE id = %s",
                    (category_id,),
                )
                conn.commit()
                conn.close()
                self.load_data()
                QMessageBox.information(
                    self, "Success", "Budget category deleted successfully!"
                )
            except Exception as e:
                QMessageBox.critical(
                    self,
    ...
    def add_purchase(self):
        ...
        try:
            ...
            elapsed = time.time() - start_time
            print(
                f"Cached data extracted in {elapsed:.2f}s - {len(accounts)} accounts, {len(categories)} categories"
            )
            ...
            QMessageBox.critical(
                self, "Database Error", f"Failed to add purchase: {str(e)}"
            )
    ...
    def process_purchase_result(self, dialog):
        ...
        try:
            ...
            self.load_data()
            QMessageBox.information(
                self, "Success", "Purchase added successfully!"
            )
    ...
    def delete_purchase(self):
        ...
        if selected_row < 0:
            QMessageBox.warning(
                self, "Selection Error", "Please select a purchase"
            )
            return
        ...
        if reply == QMessageBox.Yes:
            purchase_id = int(
                self.purchases_table.item(selected_row, 0).text()
            )
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "SELECT amount, account_id, budget_category_id FROM purchases WHERE id = %s",
                    (purchase_id,),
                )
                ...
                if purchase:
                    ...
                    self.load_data()
                    QMessageBox.information(
                        self, "Success", "Purchase deleted successfully!"
                    )
...
class AccountDialog(QDialog):
    ...
    def validate_and_accept(self):
        ...
        if not name:
            QMessageBox.warning(
                self, "Validation Error", "Account name cannot be empty!"
            )
            return
        if len(name) < 2:
            QMessageBox.warning(
...
class BudgetCategoryDialog(QDialog):
    ...
    def validate_and_accept(self):
        ...
        if not name:
            QMessageBox.warning(
                self, "Validation Error", "Category name cannot be empty!"
            )
            return
        if len(name) < 2:
            QMessageBox.warning(
...
class PurchaseDialog(QDialog):
    def __init__(self, parent, accounts, categories):
        print(
            f"PurchaseDialog.__init__ starting with {len(accounts)} accounts, {len(categories)} categories"
        )
    ...
    def validate_and_accept(self):
        ...
        if not user:
            QMessageBox.warning(
                self, "Validation Error", "User name cannot be empty!"
            )
            return

        if self.amount_spin.value() <= 0:
            QMessageBox.warning(
                self, "Validation Error", "Amount must be greater than 0!"
            )
            return
    ...
    def __init__(self, parent, accounts):
        ...
        for account in accounts:
            self.from_account_combo.addItem(
                f"{account[1]} (R{account[2]:.2f})", account[0]
            )
...
class IncomeDialog(QDialog):
    ...
    def validate_and_accept(self):
        if self.amount_spin.value() <= 0:
            QMessageBox.warning(
                self, "Invalid Amount", "Income amount must be greater than 0!"
            )
            return
        ...
        if not self.description_edit.text().strip():
            QMessageBox.warning(
                self,
                "Missing Description",
                "Please enter an income source/description!",
            )
            return
