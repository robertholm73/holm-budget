...
def check_python():
    ...
    try:
        result = subprocess.run(
            ["python", "--version"], capture_output=True, text=True, shell=True
        )
    ...
    try:
        result = subprocess.run(
            ["py", "--version"], capture_output=True, text=True, shell=True
        )
        if result.returncode == 0:
    ...
    try:
        result = subprocess.run(
            [
                "python",
                "-c",
                'import PySide6; print("PySide6 version:", PySide6.__version__)',
            ],
            capture_output=True,
            text=True,
            shell=True,
...
def check_pg8000():
    ...
    try:
        result = subprocess.run(
            ["py", "-c", 'import pg8000; print("pg8000 available")'],
            capture_output=True,
            text=True,
            shell=True,
        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0:
                    ...
                    print(
                        f"✗ Failed to install {package} with py: {result.stderr}"
                    )
...
def create_windows_launcher():
    ...
    windows_path = (
        str(current_dir).replace("/mnt/c/", "C:\\").replace("/", "\\")
    )
